...
def test_list_sessions_returns_current_session(client):
    _register_login(client, "sess@test.com", "pass1234", "sess_user")
...
def test_list_sessions_includes_fields(client):
    _register_login(client, "sess2@test.com", "pass1234", "sess_user2")
    r = client.get("/auth/sessions")
    assert r.status_code == 200
    s = r.json()[0]
    assert "id" in s
...
def test_multiple_logins_create_multiple_sessions(client):
    client.post("/auth/register", json={"email": "multi@test.com", "password": "pass1234", "username": "multi_user"})
    client.post("/auth/login", json={"email": "multi@test.com", "password": "pass1234"})
    client.post("/auth/login", json={"email": "multi@test.com", "password": "pass1234"})
    r = client.get("/auth/sessions")
    assert r.status_code == 200
    assert len(r.json()) >= 2
...
def test_revoke_other_session_returns_204(client):
    client.post("/auth/register", json={"email": "rev@test.com", "password": "pass1234", "username": "rev_user"})
    client.post("/auth/login", json={"email": "rev@test.com", "password": "pass1234"})
...
def test_revoke_current_session_clears_cookie(client):
    client.post("/auth/register", json={"email": "revcurr@test.com", "password": "pass1234", "username": "revcurr"})
    client.post("/auth/login", json={"email": "revcurr@test.com", "password": "pass1234"})
...
def test_revoke_nonexistent_session_returns_404(client):
    _register_login(client, "rev404@test.com", "pass1234", "rev404_user")
...
def test_revoke_another_users_session_returns_404(client):
    ...
    client.post("/auth/register", json={"email": "a@test.com", "password": "pass1234", "username": "user_a"})
    r_a = client.post("/auth/login", json={"email": "a@test.com", "password": "pass1234"})

    client.post("/auth/register", json={"email": "b@test.com", "password": "pass1234", "username": "user_b"})
    r_b = client.post("/auth/login", json={"email": "b@test.com", "password": "pass1234"})
    ...
    # Log in as A
    client.post("/auth/login", json={"email": "a@test.com", "password": "pass1234"})
...
def test_forgot_password_known_email_returns_token(client):
    client.post("/auth/register", json={"email": "forgot@test.com", "password": "pass1234", "username": "forgot_user"})
...
def test_reset_password_valid_token_succeeds(client):
    client.post("/auth/register", json={"email": "reset@test.com", "password": "oldpass1", "username": "reset_user"})
...
def test_reset_password_allows_login_with_new_password(client):
    client.post("/auth/register", json={"email": "reset2@test.com", "password": "oldpass1", "username": "reset_user2"})
...
def test_reset_password_old_password_rejected_after_reset(client):
    client.post("/auth/register", json={"email": "reset3@test.com", "password": "oldpass1", "username": "reset_user3"})
    token = client.post("/auth/forgot-password", json={"email": "reset3@test.com"}).json()["reset_token"]
    client.post("/auth/reset-password", json={"token": token, "new_password": "newpass456"})

    r = client.post("/auth/login", json={"email": "reset3@test.com", "password": "oldpass1"})
    assert r.status_code == 401


def test_reset_password_token_can_only_be_used_once(client):
    client.post("/auth/register", json={"email": "reset4@test.com", "password": "oldpass1", "username": "reset_user4"})
...
def test_reset_password_invalid_token_returns_400(client):
    r = client.post("/auth/reset-password", json={"token": "bad-token-xyz", "new_password": "newpass12"})
    assert r.status_code == 400
...
def test_change_password_succeeds_with_correct_current(client):
    _register_login(client, "chpw@test.com", "oldpass1", "chpw_user")
    r = client.put("/auth/password", json={"current_password": "oldpass1", "new_password": "newpass789"})
    assert r.status_code == 200
    assert r.json()["detail"] == "Password changed"


def test_change_password_allows_login_with_new(client):
    _register_login(client, "chpw2@test.com", "oldpass1", "chpw_user2")
    client.put("/auth/password", json={"current_password": "oldpass1", "new_password": "newpass789"})
...
def test_change_password_wrong_current_returns_400(client):
    _register_login(client, "chpw3@test.com", "pass1234", "chpw_user3")
    r = client.put("/auth/password", json={"current_password": "wrongpass", "new_password": "newpass12"})
    assert r.status_code == 400
    assert "incorrect" in r.json()["detail"]


def test_change_password_requires_auth(client):
    r = client.put("/auth/password", json={"current_password": "currentpass1", "new_password": "newpass1234"})
    assert r.status_code == 401
